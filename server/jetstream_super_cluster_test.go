...
func TestJetStreamSuperClusterMoveThenScaleThenCancel(t *testing.T) {
	...
	// Not back to the origin's placement, that would be a cancel. The guard fires
	// before peer selection, so the placement doesn't need to resolve to servers.
	moveCfg := *cfg
...
func TestJetStreamSuperClusterMovePlacementRevertCancels(t *testing.T) {
	sc := moveTestSuperCluster(t)
	defer sc.shutdown()
	...
	// Setting the placement back to the origin's placement cancels the move.
	cfg.Placement = &nats.Placement{Tags: []string{"C1"}}
	_, err = js.UpdateStream(cfg)
	require_NoError(t, err)

	// If the move converged before this landed, it started a fresh move back rather than
	// canceling. The assertions below can't tell those apart, so catch it here.
	peers, _, _ := moveTestStreamGroup(t, ml, globalAccountName, "TEST")
	for _, peer := range originPeers {
		if !slices.Contains(peers, peer) {
			t.Fatalf("Move converged before the placement revert landed, cancel path not exercised: "+
