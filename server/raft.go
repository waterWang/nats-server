...
func (n *raft) runAsLeader() {
	...
	for n.State() == Leader {
		...
		case <-n.prop.ch:
			const (
				maxEntries   = 4096 // larger batches showed no benefit
				batchBaseLen = appendEntryBaseLen + binary.MaxVarintLen64
			)
...
retry:
	...
	if pstate == Leader && state != Leader {
		leadChanged = true
		n.updateLeadChange(false)
		// Disable fast path proposals and wait for any active proposal
		// to finish before draining the proposal queue below.
		n.closeProposalFastPath()
